<?php class EtRecentVideosWidget extends WP_Widget
{
	function EtRecentVideosWidget(){
		...
		parent::WP_Widget( false, $name = __( 'Videos Recentes', 'Nexus' ), $widget_ops, $control_ops );
	}
	...
	function widget($args, $instance){
		...
		$et_category_number = empty($instance['et_category_number']) ? '' : (int) $instance['et_category_number'];
		...
		$et_recent_videos_query = new WP_Query( apply_filters( 'et_recent_videos_args', array(
			...
			'orderby' => $select,
			'cat' => (int) $et_category_number, #category 18 = Aulas
			'tax_query' => array(
				array(
...
<?php
	...
	function update($new_instance, $old_instance){
		...
		$instance['et_category_number'] = (int) $new_instance['et_category_number'];
	...
	function form($instance){
		//Defaults
		$instance = wp_parse_args( (array) $instance, array( 'title' => __( 'Recent Videos', 'Nexus' ), 'et_recent_posts_number' => 4, 'et_category_number' => 4 ) );
		$title = esc_attr( $instance['title'] );
		$et_recent_posts_number = (int) $instance['et_recent_posts_number'];
		$et_category_number = (int) $instance['et_category_number'];
		$select = esc_attr($instance['select']); // Added 
		# Title
		...
		echo '<p><label for="' . esc_attr( $this->get_field_id('et_category_number') ) . '">' . 'Category:' . '</label><input class="widefat" id="' . esc_attr( $this->get_field_id('et_category_number') ) . '" name="' . esc_attr( $this->get_field_name('et_category_number')  ). '" type="text" value="' . esc_attr( $et_category_number ) . '" /></p>';
		

echo '<p><label for="' . $this->get_field_id('select') . '">' . _e('Post Order:', 'wp_widget_plugin') . '</label>';
